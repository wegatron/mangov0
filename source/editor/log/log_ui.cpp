...
namespace mango {
void LogUI::init() { m_title = "Log"; }

void LogUI::construct() {
  std::vector<std::string> lastest_logs =
      g_engine.logSystem()->getLastestLogs();

  ImGuiWindowFlags window_flags = 0;
  window_flags |= ImGuiWindowFlags_HorizontalScrollbar;
  sprintf(m_title_buf, "%s %s###%s", ICON_FA_RECEIPT, m_title.c_str(),
          m_title.c_str());
...
void LogUI::destroy() { EditorUI::destroy(); }

size_t find_nth(const std::string &haystack, const std::string &needle,
                size_t nth) {
  size_t pos = -1;
  int cnt = 0;
...
void LogUI::showLogText(const std::string &log) {
  ...
  for (int i = 0; i <= (int)(ELogLevel::Fatal); ++i) {
    ...
    if (std::string(log.begin() + 1, log.begin() + log_str_len + 1) ==
        k_log_strs[log_level]) {
      ...
      ImGui::Text(
          std::string(log.begin() + second_right_bracket_pos + 2, log.end())
              .c_str());
    }
  }
}

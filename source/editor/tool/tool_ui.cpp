...
void ToolUI::init() {
  ...
  entity_categories = {std::string(ICON_FA_CUBE) + " Entities",
                       std::string(ICON_FA_LIGHTBULB) + " Lights",
                       std::string(ICON_FA_SHAPES) + " Primitives"};

  entity_typess = {{std::string(ICON_FA_ASTERISK) + " Empty Entity"},
  ...
  const auto &entity_class_names =
      g_engine.worldManager()->getCurrentWorld()->getEntityClassNames();
  for (const std::string &entity_class_name : entity_class_names) {
    entity_typess.front().push_back(std::string(ICON_FA_ASTERISK) + " " +
                                    entity_class_name);
  }
}
...
void ToolUI::construct() {
  ...
  sprintf(m_title_buf, "%s %s###%s", ICON_FA_TOOLS, m_title.c_str(),
          m_title.c_str());
  ...
  const char *play_icon_text =
      current_world_mode == EWorldMode::Edit
          ? ICON_FA_PLAY
          : (current_world_mode == EWorldMode::Play ? ICON_FA_PAUSE
                                                    : ICON_FA_FORWARD);
      ...
      if (ImGui::BeginMenu(entity_categories[i].c_str())) {
        for (size_t j = 0; j < entity_typess[i].size(); ++j) {
          ...
          if (ImGui::BeginDragDropSource(
            ...
            std::string playload_str =
                std::string(entity_categories[i].begin() + 4,
                            entity_categories[i].end()) +
                "-" + std::string(entity_type.begin() + 4, entity_type.end());
            ImGui::SetDragDropPayload("create_entity", playload_str.data(),
                                      playload_str.size());

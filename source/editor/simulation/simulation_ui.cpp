...
void SimulationUI::init() {
  m_title = "Simulation";
  m_color_texture_sampler = VulkanUtil::createSampler(
      VK_FILTER_LINEAR, VK_FILTER_LINEAR, 1, VK_SAMPLER_ADDRESS_MODE_REPEAT,
...
void SimulationUI::construct() {
  ...
  sprintf(m_title_buf, "%s %s###%s", ICON_FA_GAMEPAD, world_name.c_str(),
          m_title.c_str());
...
void SimulationUI::destroy() {
  ...
  vkDestroySampler(VulkanRHI::get().getDevice(), m_color_texture_sampler,
                   nullptr);
...
void SimulationUI::onWindowResize() {
  ...
  m_color_texture_desc_set = ImGui_ImplVulkan_AddTexture(
      m_color_texture_sampler, g_engine.renderSystem()->getColorImageView(),
...
void SimulationUI::constructImGuizmo() {
  ...
  if (m_operation_mode != EOperationMode::Pick) {
    ...
    ImGuizmo::Manipulate(p_view, p_projection, operation,
                         ...
                         glm::value_ptr(matrix), glm::value_ptr(delta_matrix),
...
void SimulationUI::updateCamera() {
  ...
  m_camera_component.lock()->m_aspect_ratio =
      (float)m_content_region.z / m_content_region.w;
  if (g_engine.isSimulating()) {
...
glm::vec3 SimulationUI::calcPlacePos(const glm::vec2 &mouse_pos,
  ...
  glm::vec3 ray_origin =
      glm::unProjectZO(glm::vec3(mouse_pos.x, mouse_pos.y, 0.0f),
                       ...
                       glm::vec4(0.0f, 0.0f, viewport_size.x, viewport_size.y));
  glm::vec3 ray_target =

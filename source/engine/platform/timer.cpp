...
void TimerManager::init() { m_time = 0.0f; }
...
void TimerManager::destroy() { m_timers.clear(); }

TimerHandle TimerManager::addTimer(float interval,
                                   const std::function<void(void)> &func,
                                   bool loop, bool loop_im_call) {
  if (loop_im_call) {
    func();
  }
...
long long StopWatch::stopMs() {
  m_end_time = std::chrono::high_resolution_clock::now();
  return std::chrono::duration_cast<std::chrono::milliseconds>(m_end_time -
                                                               m_start_time)
      .count();
}
...
float StopWatch::stop() {
  m_end_time = std::chrono::high_resolution_clock::now();
  return std::chrono::duration_cast<std::chrono::nanoseconds>(m_end_time -
                                                              m_start_time)
             .count() *
         1e-9f;
}
...
void TimeOuter::trigger() { trigger_timestamp = getTimestampMs(); }

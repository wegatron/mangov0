...
void FileSystem::init() {
  if (std::filesystem::exists(std::filesystem::path("asset"))) {
    ...
    throw std::runtime_error("failed to find engine asset");
  }
...
std::string FileSystem::relative(const std::string &path) {
  std::string rel_path =
      std::filesystem::relative(path, m_header).generic_string();
  return rel_path;
}
...
std::vector<std::string> FileSystem::traverse(const std::string &path,
                                              bool is_recursive,
                                              EFileOrderType file_order_type,
                                              bool is_reverse) {
  ...
  std::sort(filenames.begin(), filenames.end(),
            [file_order_type, is_reverse](const std::string &lhs,
              ...
              case EFileOrderType::Time:
                result = std::filesystem::last_write_time(lhs) <
                         std::filesystem::last_write_time(rhs);
                break;
              case EFileOrderType::Size:
                result = std::filesystem::file_size(lhs) <
                         std::filesystem::file_size(rhs);
                break;
              default:
                break;
              }
...
std::string FileSystem::validateBasename(const std::string &basename) {
  ...
  for (size_t i = 0; i < basename.size(); ++i) {
    bool is_valid_char = std::find(invalid_chars.begin(), invalid_chars.end(),
                                   basename[i]) == invalid_chars.end();
    validated_basename.push_back(is_valid_char ? basename[i] : '_');
  }
  return validated_basename;
}

std::string FileSystem::getAssetDir() { return absolute("asset"); }

std::string FileSystem::getShaderDir() { return absolute("shader"); }

std::string FileSystem::getSpvDir() { return absolute("asset/engine/shader"); }

std::string FileSystem::getLogDir() { return absolute("log"); }

std::string FileSystem::getCacheDir() { return absolute("cache"); }

bool FileSystem::exists(const std::string &path) {
  return std::filesystem::exists(path) ||
         std::filesystem::exists(absolute(path));
}
...
void FileSystem::renameFile(const std::string &dir, const std::string &old_name,
                            const std::string &new_name) {
  if (old_name.compare(new_name)) {
    try {
      std::filesystem::rename(dir + old_name, dir + new_name);
    } catch (const std::filesystem::filesystem_error &e) {
      LOGW("rename file error: {}", e.what());
    }
  }
}

bool FileSystem::loadBinary(const std::string &filename,
                            std::vector<uint8_t> &data) {
  std::ifstream file(filename, std::ios::ate | std::ios::binary);
  if (!file.is_open()) {
    LOGF("failed to load binary file {}", filename);
    return false;
  }
...
bool FileSystem::writeString(const std::string &filename,
                             const std::string &str) {
  std::ofstream file(filename);
  if (!file.is_open()) {
    LOGF("failed to write string file {}", filename);
    return false;
  }
...
bool FileSystem::loadString(const std::string &filename, std::string &str) {
  ...
  if (!file.is_open()) {
    LOGF("failed to load string file {}", filename);
    return false;
  }

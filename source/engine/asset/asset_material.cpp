...
void Material::inflate() {
  ...
  auto [desc_set, buffer, offset] =
      resource_binding_mgr->requestStandardMaterial();
  buffer->update(&material_, sizeof(UMaterial), offset);

  VkDescriptorBufferInfo desc_buffer_info{.buffer = buffer->getHandle(),
                                          .offset = offset,
                                          .range = sizeof(UMaterial)};
  auto driver = g_engine.getDriver();
  auto sampler = g_engine.getResourceCache()->requestSampler(
      driver, VkFilter::VK_FILTER_LINEAR, VkFilter::VK_FILTER_LINEAR,
      VkSamplerMipmapMode::VK_SAMPLER_MIPMAP_MODE_LINEAR,
      VkSamplerAddressMode::VK_SAMPLER_ADDRESS_MODE_REPEAT,
      VkSamplerAddressMode::VK_SAMPLER_ADDRESS_MODE_REPEAT)->getHandle();
  
  VkDescriptorImageInfo desc_image_infos[] = {
      {.sampler = sampler,
       .imageView = albedo_texture_ == nullptr ? VK_NULL_HANDLE : albedo_texture_->getImageView()->getHandle(),
       .imageLayout =
           VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL}, // binding point 1
      {.sampler = sampler,
       .imageView = normal_texture_ == nullptr ? VK_NULL_HANDLE : normal_texture_->getImageView()->getHandle(),
       .imageLayout =
           VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL}, // binding point 2
      {.sampler = sampler,
       .imageView = emissive_texture_ == nullptr ? VK_NULL_HANDLE : emissive_texture_->getImageView()->getHandle(),
       .imageLayout =
           VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL}, // binding point 3
      {.sampler = sampler,
       .imageView = metallic_roughness_occlution_texture_ == nullptr ? VK_NULL_HANDLE : metallic_roughness_occlution_texture_->getImageView()->getHandle(),
       .imageLayout =
